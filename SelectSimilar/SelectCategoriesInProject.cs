...
namespace SelectSimilar
{
    ...
    class SelectCategoriesInProject : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                //Select elements
                uidoc.Selection.SetElementIds(new FilteredElementCollector(uidoc.Document).WhereElementIsNotElementType().Where(a => a.Category != null && catids.Contains(a.Category.Id.IntegerValue)).Select(a => a.Id).ToList());
