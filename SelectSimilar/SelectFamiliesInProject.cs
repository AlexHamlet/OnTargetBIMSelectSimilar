...
namespace SelectSimilar
{
    ...
    class SelectFamiliesInProject : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                select = select.Union(new FilteredElementCollector(uidoc.Document).WhereElementIsNotElementType().Where(a => typelessnames.Contains(a.Name)).Select(a => a.Id)).ToList();
